...
for (i in 1:ncov) x[,i] <- runif(n = size, min = 0, max = 1)
x <- cbind(1, x)
mu_true <- x %*% true_beta

#-------------Error contamination probability----------------#
contaim_prob = 0 # 0.1/0.2/0.3
...
sim_omegaparam_beta <- function(thechosenseed){ 
  ...
  error_true<- matrix(nrow = size, ncol = 1)
  y<- matrix(nrow = size, ncol = 1)
  
  # Contaminated error
  # N(0,9)
  error_true[1:(size*contaim_prob), ]<- rnorm(n = size*contaim_prob, mean = 0, sd = 3) 
  # t(2)
  # error_true[1:(size*contaim_prob), ]<- sigma_true*rt(n = size*contaim_prob, df = nu_true) 
  # chi^2(4) - 4
  # error_true[1:(size*contaim_prob), ]<- rchisq(n = size*contaim_prob, df = 4) - 4 
  
  y[1:(size*contaim_prob), ]<-mu_true[1:(size*contaim_prob), ] + error_true[1:(size*contaim_prob), ]
  
  # Two point contamination: prob/2
  # y[1: (size*contaim_prob/2), ] <- rep(5, size*contaim_prob/2)
  # y[(size*contaim_prob/2+1): (size*contaim_prob), ] <- rep(-5, size*contaim_prob/2)
  
  # Uncontaminated error
  error_true[(size*contaim_prob + 1):size, ] <- rnorm(n = size*(1-contaim_prob), mean = 0, sd = sigma_true) # N(0,1)
  
  y[(size*contaim_prob + 1):size, ] <- mu_true[(size*contaim_prob + 1):size, ] + error_true[(size*contaim_prob + 1):size, ]
