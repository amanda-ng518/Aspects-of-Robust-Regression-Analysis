...
#--------------Load dataset-------------#
stackloss_data = stackloss # x = 1:3
...
# cov_X <- cov(X_orig)
# new_X <- mvrnorm(n = size - nrow(X_orig), mu = mu_X, Sigma = cov_X)
# X_simulated <- rbind(X_orig, new_X)
# x <- matrix(nrow = size, ncol = ncov)
# x[, 1:3] <- X_simulated
# for (i in 4:ncov) {
#   x[, i] <- rnorm(size)  # Fill remaining columns with standard normal 
# }
# true_beta <- append(true_beta, rep(0,ncov+1-4))
...
sim_omegaparam_beta <- function(thechosenseed){ 
  ...
  #------------Huber regression-----------#
  RB <- rlm(y ~ x[,-1], psi = psi.huber, k = 1.345)
